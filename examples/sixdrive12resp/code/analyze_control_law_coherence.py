...
def per_channel_rms(cpsd, freqs, band=None):
    ...
    REFERENCE_DRIVE_LEVEL_VRMS=20.0 / "2x reference"=40.0 calibration
    points (see build_nonlinear_frf_system_allmodes.py) rather than
    guessing from the GUI's Target Test Level dB alone, since that
    idealized calibration assumed a single mode-shaped sine, not a real
    broadband random spec.
    """
...
def main():
    ...
    print(f"Per-channel drive RMS voltage ({band_str}) -- compare against the nonlinear\n"
          f"system's REFERENCE_DRIVE_LEVEL_VRMS=20.0 ('1x reference', modest engagement)\n"
          f"and 'x2 reference'=40.0 (full calibrated target shift) design points:\n")
